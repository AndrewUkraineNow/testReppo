...
GivenStories:/story/precondition/Precondition.story#{group:createBoardAndLogin}
Lifecycle:
Examples:
...
Given I initialize STORY variable `url` with value `${api_url}/1/members/me/boards?fields=id,name&name=${board_name}&key=${key}&token=${token}`
...
Given I initialize STORY variable `url2` with value `${api_url}/1/boards/#{removeWrappingDoubleQuotes(${id_board})}?key=${key}&token=${token}`
